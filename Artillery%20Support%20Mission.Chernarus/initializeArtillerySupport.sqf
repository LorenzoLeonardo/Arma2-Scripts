...
_targetPos = 0;
_countAmmo = 0;
bArtilleryBusyAlpha = false;//Lock Artillery flag when someone is still active using it.
...
while { (_countAmmo > 0) && (_countEast > 0) && ( count _artilleryUnit > 0) } do
{
	...
	if(	_countEast >= 1) then
	{
		...
		if( _countEnemyMax >= 9 ) then
		{
			...
			if( !bArtilleryBusyAlpha ) then
			{
				bArtilleryBusyAlpha = true;
